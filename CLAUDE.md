...
- **Costanti** in `SCREAMING_SNAKE_CASE` (es. `INSERT_BATCH_SIZE`, `DATA_START_ROW_INDEX`).
- **Lettura header Excel resiliente:** le colonne si individuano per **nome header**, non per posizione fissa (gli export variano in ordine/numero di colonne). Gli header si normalizzano (`normalize_header` in `utils.rs`) e si accettano più alias italiani/inglesi tramite `build_header_map` / `find_required_header` / `find_optional_header` (`utils.rs`, condivisi tra `service/customer.rs` e `service/product.rs`).
...
- **Da verificare con i template/scheletri REALI** (in `src-tauri/resources/` ora ci sono i campioni ADM vuoti): righe di inizio dati, quali colonne sono formule (col `P`), layout reale di `skeleton_pat` (code col Q, desc col M, adm col L). Il frontend manda una **fine quindicina** come data ISO (`generate_tracciati` param `fortnight_end`, selezionata da una lista che propone 15 e ultimo giorno del mese, default alla prossima): PAT scrive una **data Excel reale** (`CellValue::Date` → serial + formato `dd/mm/yyyy`) in "Data fine quindicina", PLI scrive solo il mese `MM/YYYY` in "Data mese".
- **Ownership campi prodotto:** import possiede `{product_type, units, packages}` e, solo per PLI, `adm_code` (derivato da `code`, vedi sotto); scheletro possiede `{description, capacity, nicotine, tabella}` e, solo per PAT, `adm_code` (colonna dedicata dello scheletro). La UPSERT di re-import (`build_product_batch_insert_sql`) aggiorna i campi import più `adm_code` via `COALESCE(excluded.adm_code, product.adm_code)` — per PAT l'import passa sempre `None` quindi il valore dello scheletro non viene mai toccato; lo scheletro fa UPDATE-by-code (`update_products_from_skeleton`) con COALESCE per non azzerare l'altro lato. I campi dello scheletro restano **NULL** all'import (niente più placeholder `0`) finché non arriva lo scheletro. `gruppo` serve solo a classificare PLI/PAT; il valore `tabella` viene da `skeleton_pat`.
- **Eccezione `adm_code` PLI:** un codice prodotto PLI può terminare con una singola lettera **D**/**K**/**S** dopo la parte numerica (es. `PL0012162D`) che va rimossa per ottenere il codice ADM (`PL0012162`). La regola (`service/product::pli_adm_code`) si applica all'**import** (non allo scheletro): la lettera va rimossa solo se precede una cifra; senza quel suffisso `adm_code` resta `None` e in `tracciati_pli` si usa `code` come fallback (`service/excel::build_pli_row`).
